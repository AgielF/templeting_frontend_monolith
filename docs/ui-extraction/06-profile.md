...
## Sumber
- Path terpilih: `app/Views/auth/profile.php`
- Ukuran: 284 baris
- Kandidat dibandingkan: `app/Views/profile_view.php` (0 baris), `app/Views/user_profile_view.php` (9 baris), `app/Views/sections/user_profile.php`.
- Alasan pilihan: `auth/profile.php` paling lengkap dan dipakai controller `AuthUi::profile()`; `profile_view.php` kosong.
- Asset yang di-link: Font Awesome, asset foto/profile bila tersedia.
...
|---|---|---|
| `$user` | `AuthUi::profile()`/database | Detail user dan role |
| `role_id_label` | Controller/view data | Label peran |
| `$errors`/flashdata | Session/validation | Pesan update profile |

## Form & Aksi
- Form update profile berada di view lengkap.
- Metode POST ke aksi update profile SSIP; field mengikuti user model SSIP.

## Struktur HTML Penting
- Kartu identitas, detail role/nomor, ikon kontak, form edit profile.

## Catatan A11y Saat Ini
- label for/id: perlu audit pada form panjang.
- aria-*: perlu ditambahkan pada ikon-only action.
- role: modal/dialog bila ada harus memakai `role="dialog"`.
- Heading hierarchy: audit heading profile.

## Catatan Branding SSIP
- Identitas pengguna dan nama kampus.
- Wajib diganti saat porting: brand ya; data user dipetakan ke session target.