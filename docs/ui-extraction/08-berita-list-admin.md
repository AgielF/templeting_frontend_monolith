...
## Sumber
- Wrapper: `app/Views/berita_list_admin_view.php` (6 baris)
- UI utama: `app/Views/sections/berita_list_admin.php` (253 baris)
- Asset yang di-link: Bootstrap, Font Awesome, JavaScript modal/toast.
...
<?= $this->include('sections/berita_list_admin') ?>

<?= $this->include('layout/footer') ?>
```

Kode section utama berikut disalin verbatim dari `app/Views/sections/berita_list_admin.php`:
...
|---|---|---|
| `$schedules` | `BeritaController::admin()` | Hasil query berita + creator |
| `$row['id_berita']` | `BeritaModel` | ID aksi edit/delete |
| `$row[...]` | Query join users | Field tabel dan data modal |

## Form & Aksi
- Delete: POST `/berita/delete/{id_berita}`, CSRF, konfirmasi JavaScript.
- Tambah: POST `/berita/store`, field `judul`, `konten`, `kategori`, `tanggal`.
- Edit: form POST dinamis, field hidden `id_berita` dan empat field berita.

## Struktur HTML Penting
- Tabs, card, responsive table, toast, modal tambah/edit, pagination client-side/filter controls.
- Ikon edit, delete, PDF, plus dari Font Awesome.

## Catatan A11y Saat Ini
- label for/id: ada pada search/sort/items-per-page; modal field label belum semua punya `for`.
- aria-*: toast memiliki live region; close modal perlu label.
- role: toast alert tersedia; modal perlu `role="dialog"`.
- Heading hierarchy: h4/h5 perlu disesuaikan dengan main heading.

## Catatan Branding SSIP
- `Daftar Berita`, `Tambah Berita`, dan pesan Bahasa Indonesia.
- Wajib diganti saat porting: logo/brand; teks modul dapat dipertahankan.
